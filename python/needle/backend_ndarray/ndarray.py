...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        if prod(new_shape) != prod(self._shape):
            raise ValueError("SHAPE ERROR: Product of new shape must equal product of current shape")
        if not self.is_compact():
            raise ValueError("Can't in-place reshape non-compact array")
        return NDArray.make(
            new_shape,
    ...
    def permute(self, new_axes: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        assert len(new_axes) == self.ndim, f"new_axes must have ndim length, it has {len(new_axes)} instead of {self.ndim}"
        assert set(new_axes) == set(range(self.ndim)), f"new_axes must be a permutation of dimension indices, got {new_axes}"
        
        # Permute both shape and strides according to new_axes
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        assert len(new_shape) >= self.ndim, f"new_shape must have at least as many dimensions as original, got {new_shape} < {self.ndim}"
        shape_diff = len(new_shape) - self.ndim
        padded_shape = (1,) * shape_diff + self._shape
        padded_strides = (0,) * shape_diff + self._strides

        new_strides = []
        for old_dim, new_dim, stride in zip(padded_shape, new_shape, padded_strides):
            if old_dim == new_dim:
                # Dimension stays the same, keep stride
                new_strides.append(stride)
            elif old_dim == 1:
                # Broadcasting dimension of size 1: set stride to 0
                new_strides.append(0)
            else:
                raise AssertionError(f"Cannot broadcast dimension of size {old_dim} to {new_dim}")
    ...
    def squeeze(self, axis: int | tuple[int, ...] | list[int] | None = None) -> "NDArray":
        ...
        Args:
            axis (int | tuple[int, ...] | list[int] | None): The axis or axes to squeeze.
                If None, all dimensions of size 1 are removed.
                If an axis is specified, it will only be removed if its size is 1.

        Returns:
            NDArray: A new view of the array with squeezed dimensions.
        """
        ...
        axes_to_squeeze = []
        if axis is None:
            # Find all axes of size 1
            axes_to_squeeze = [i for i, size in enumerate(self.shape) if size == 1]
        else:
            # Normalize specified axes
            axes_tuple = (axis,) if isinstance(axis, int) else tuple(axis)
            axes_to_squeeze_normalized = []
            for ax in axes_tuple:
                ax_norm = ax if ax >= 0 else self.ndim + ax
                if not 0 <= ax_norm < self.ndim:
                    raise ValueError(f"axis {ax} is out of bounds for array of ndim {self.ndim}")
                if self.shape[ax_norm] != 1:
                    raise ValueError(f"cannot squeeze axis {ax_norm} with size {self.shape[ax_norm]} != 1")
                axes_to_squeeze_normalized.append(ax_norm)
            axes_to_squeeze = axes_to_squeeze_normalized
            
        # Remove axes in reverse order to not mess up indexing
        axes_to_squeeze.sort(reverse=True)
        for ax in axes_to_squeeze:
            new_shape.pop(ax)
            new_strides.pop(ax)

        # Handle case where all dims are squeezed, resulting in a 0D array
        if not new_shape and self.size == 1:
            return NDArray.make((), strides=(), device=self.device, handle=self._handle, offset=self._offset)
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for i, sl in enumerate(slices):
            # Update offset: move starting position by start * stride
            new_offset += sl.start * self._strides[i]
            
            # Calculate new dimension size: number of elements in slice
            # Formula: ceil((stop - start) / step) = (stop - start + step - 1) // step
            dim_size = (sl.stop - sl.start + sl.step - 1) // sl.step
            new_shape.append(dim_size)
            
            # Update stride: multiply by step (if step=2, skip every other element)
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        for axis in axes:
            ...
            if not 0 <= ax < self.ndim:
                raise ValueError(f"Axis {axis} out of bounds for array of ndim {self.ndim}")
                
            # To flip an axis:
            # 1. Negate its stride
            # 2. Adjust the offset to point to the new 'first' element,
            #    which is the original last element along that axis.
            dim_size = self._shape[ax]
            old_stride = self._strides[ax]
            
            new_offset += (dim_size - 1) * old_stride
            new_strides[ax] = -old_stride
            
        # Create a view with negative strides
        view = NDArray.make(
            self._shape,
            strides=tuple(new_strides),
            device=self._device,
        ...
        # Per the note: compact() the array immediately.
        # This copies the data from the (unsupported) negative-stride view
        # into a new, (supported) positive-stride compact array.
        return view.compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert len(axes) == self.ndim, "Padding 'axes' must have one entry per dimension"
        
        new_shape = []
        slices_for_setitem = []
        
        for i in range(self.ndim):
            pad_left, pad_right = axes[i]
            assert pad_left >= 0 and pad_right >= 0, "Padding must be non-negative"
            
            original_dim = self.shape[i]
            
            # 1. Calculate the new, larger shape
            new_shape.append(original_dim + pad_left + pad_right)
            
            # 2. Calculate the slice where the original data will be copied
            #    e.g., if pad_left=1, data starts at index 1
            slices_for_setitem.append(slice(pad_left, pad_left + original_dim))

        # 3. Create a new, zero-filled array with the larger shape
        #    We use self.device.full, which is available on the BackendDevice
        out = self.device.full(tuple(new_shape), 0.0, dtype=self.dtype)

        # 4. Use __setitem__ to copy self's data into the correct 'window'
        out[tuple(slices_for_setitem)] = self
